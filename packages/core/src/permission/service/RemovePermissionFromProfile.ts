  ...
  async execute(data: PermissionProfileProps, user?: UserProps): Promise<CoreResponse> {
    ...
    if (user && user.email) {
      ...
      if (user.email) {
        ...
        console.log("permissionExistOnProfile:", permissionExistOnProfile)
        ...
        if (errors.length > 0) {
          return {
            ...
            status: 400,
            message: "Erro de validação",
            errors: [...errors, "Não é possível desvincular a permissão do perfil"]
          }
        }

        const newPermissionOnProfile = await this.repo.removePermissionFromProfile(data.permissionId, data.profileId)
